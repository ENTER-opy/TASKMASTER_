...
def give_boss_reward(data, level_number):
    xp_gain= data["rewards"][level_number]
    print(f"For defeating {data["bosses"][level_number]}, you get {data["rewards"][level_number]} gold! and {data["player"].update({"xp":xp_gain})} XP!")
    level_up(data, xp_gain)
